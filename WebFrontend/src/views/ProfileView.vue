...
async function deleteSequence(){
  ...
  if (deleteButtonStatus.value == "Delete Sequence"){
    ...
    axios.post(backendLink+"/api/deleteSequence", {cookie: Cookies.get('session_cookie'), sequence_id:currentSequence.value})
    .then(response =>{
...
async function addToSequenceList() {
  ...
  for (let sequence of serverData) {
    sequenceDropdown.value.push({ value: sequence[1], id: sequence[0]});
  }
...
<template>
  ...
  <div class="absolute w-full h-100 md:h-95 top-20 flex justify-center items-center ">
    <div class="w-11/12 max-w-250 h-full shadow-md bg-white rounded-md flex justify-center items-center">
      
      <div class="w-full md:w-3/5 h-full grid grid-cols-1 place-items-center">

        <div class=" w-11/12 md:h-24 flex items-center justify-between">
          <p class="text-4xl font-bold"> Account Info </p>
          <div class="btn text-lg" @click="Logout();router.push({name: 'login'})">Log Out 
            <svg class="w-4 h-5" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" width="256" height="256" viewBox="0 0 256 256" xml:space="preserve">
            <defs>
            </defs>
        ...
        <fieldset class="fieldset w-11/12 rounded-md">
          <label class="fieldset-label">Name</label>
          <label class="w-full">
            <div class="w-full flex justify-center items-center gap-2">
              <input ref="nameInput" id="nameInput" v-model="accountName" disabled  type="text" @change="CheckAll" class="input input-primary w-full" />

              <div v-if="_nameInput" class="tooltip" data-tip="Edit">
                <div @click="ToggleInput('name')" class="btn btn-outline btn-warning w-12">            
                  <svg width="800px" height="800px" viewBox="0 0 16 16" fill="currentColor" xmlns="http://www.w3.org/2000/svg" class="w-6 h-6">
          ...
          <label class="w-full">
            <div class="w-full flex justify-center items-center gap-2">
              <input ref="emailInput" id="emailInput" v-model="accountEmail" disabled  type="text" @change="CheckAll" class="input input-bordered input-primary w-full" />
          ...
          <label class="w-full">
            <div class="w-full flex justify-center items-center gap-2">
              <input ref="passwordInput" id="passwordInput" v-model="accountPassword" disabled type="password" @change="CheckAll" class="input input-bordered input-primary w-full" />
          ...
          <div class="py-1"></div>
          <button class="btn btn-accent w-full md:hidden" @click="ChangeAccountInfo">
      ...
      <div class="hidden md:grid w-2/5 h-full grid-cols-1 place-items-center">
        ...
        <button class="btn btn-accent h-14 w-64" @click="ChangeAccountInfo">
          <p class="px-2">{{ changeButtonStatus }}</p>
            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-fill" viewBox="0 0 16 16">
              <path d="M12.854.146a.5.5 0 0 0-.707 0L10.5 1.793 14.207 5.5l1.647-1.646a.5.5 0 0 0 0-.708zm.646 6.061L9.793 2.5 3.293 9H3.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.5h.5a.5.5 0 0 1 .5.5v.207zm-7.468 7.468A.5.5 0 0 1 6 13.5V13h-.5a.5.5 0 0 1-.5-.5V12h-.5a.5.5 0 0 1-.5-.5V11h-.5a.5.5 0 0 1-.5-.5V10h-.5a.5.5 0 0 1-.175-.032l-.179.178a.5.5 0 0 0-.11.168l-2 5a.5.5 0 0 0 .65.65l5-2a.5.5 0 0 0 .168-.11z"/>
            </svg> 
